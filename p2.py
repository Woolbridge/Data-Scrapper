...
# Enhanced WebManipulator class
class EnhancedWebManipulator:
    ...
    def rotate_ip(self):
        if self.proxies_manager.proxies:
            ...
            chrome_options.add_argument(f'user-agent={self.generate_user_agent()}')
    ...
    def move_cursor_randomly(self, element):
        try:
            # Simulate more natural cursor movements
            speed_x = random.uniform(0.2, 1.8)
            speed_y = random.uniform(0.2, 1.8)
            action = ActionChains(self.driver)
            action.move_to_element_with_offset(element, speed_x, speed_y).perform()
            time.sleep(random.uniform(0.5, 2.0))
            # Add occasional clicks on elements
            if random.random() < 0.1:
                action.click().perform()
        except Exception as e:
            print(f"Exception during cursor movement: {e}")
    ...
    def web_manipulation_iteration(self):
        try:
            ...
            addd=math.trunc(random.uniform(1,3))
            self.cycle_count += addd

            # Stop and close the process after every four cycles
            ...
            print(f"Exception during web manipulation: {e}")
        finally:
            if self.driver:
                self.driver.quit()
...
if __name__ == '__main__':
    with Manager() as manager:
        ...
        enhanced_web_manipulator = EnhancedWebManipulator("https://foulabook.com/ar/book/%D8%A3%D8%B1%D8%B6-%D8%B2%D9%8A%D9%83%D9%88%D9%84%D8%A7-pdf?lang=fr#!comment=810", 3, 9 * 60, proxies_manager)
        processes = []

        for _ in range(5):  # Change 5 to the desired number of processes
            process = Process(target=enhanced_web_manipulator.run)
            process.start()
